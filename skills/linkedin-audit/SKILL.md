...
- `otw:remove=A,B;add=C,D;removeloc=E;addloc=F,G` — edit Open-to-work job titles AND remote locations at `opportunities/job-opportunities/edit/`. Titles MUST come from LinkedIn's fixed taxonomy — free text never commits (e.g. "Senior Frontend Engineer" does not exist; "Senior Web Developer" does). Probe candidates in the typeahead before assuming. Two form variants alternate (Add-title button + typeahead vs. always-visible chips input). Removal matches aria-label by substring, case-insensitively, across `button` AND `[role="button"]` — remove before adding, or "Frontend Developer" will match a "Senior Frontend Developer" chip. Caveats: the chip label can DIFFER from the profile's display text (profile says "Artificial Intelligence Engineer", the chip says "AI Engineer" — screenshot the form before concluding a title is absent); diacritics must match exactly (ș U+0219 vs ş U+015F); a city-level chip like "Iași, Romania" substring-matches a country add ("Romania") and falsely skips it — remove the city chip first. Remote locations should be country/region level ("Romania", "European Union"), never a city, or recruiters filtering remote roles by country will not see the profile.
...
- `skilltop:<name>` — move a skill to the top of the skills list. LinkedIn has NO reorder control for skills (verified Jul 2026: the per-skill edit form holds only association checkboxes + Delete skill + Save); list order is newest-added first, so this deletes and immediately re-adds, harvesting the checked "where did you use it" associations and re-checking them in the add form. **Never use on a skill with endorsements or a passed-assessment badge — both are permanently lost on delete** (assessments are retired, so a badge can never be re-earned). Caveat: association-row labels resolve to the SECTION header ("Experience") via label/aria, so the harvester climbs ancestors for the real row text; if a run logs `restored 0/N`, restore via `editskill:` using the before-delete screenshot as ground truth.
- `certadd:<name>|<org>|<issueMonth>|<issueYear>|<credentialId>|<credentialUrl>` — add a NEW certification (org picked from the typeahead). Month may be empty, but LinkedIn renders a year-only issue date as "Jan <year>". The new cert lands at the TOP of the list (newest-added first) — follow with `certtop:` on the rightful #1 if that disturbs the ordering.
- `edudelete:<match>` — delete an education entry, matched against the edit form's FIELD VALUES (the school name lives in an input). When moving a short-course cert out of Education, run `certadd:` first and delete only after the cert saved.
- `expadd:<titleFragment>` — add a NEW position (from `changes.experienceAdds: [{title, company, employmentType, current, startMonth, startYear, locationType, description}]`) via the directly navigable `edit/forms/position/new/` overlay. THREE known side effects, all seen on one Aug 2026 run — verify all three after every use: (1) the company typeahead can link a same-named but WRONG company page (an "ISIO" studio got linked to Isio, a UK pensions firm — fix with `expcompany:`); (2) LinkedIn silently REPLACES the custom headline with "<Title> at <Company>" when a current position is added — re-run the `headline` op after; (3) the start-date selects can keep LinkedIn's stale prefill (a previous form's dates) — verify and fix with `expdate:`. The post-save "Position saved — verify you work at X" dialog contains an email input; it is a SUCCESS state, not a stuck form.
- `expcompany:<formMatch>@<newCompany>` — relink a position's company field to a different company page (typeahead match required; free text saves but drops the logo). Matches the position by its edit form's FIELD VALUES.
- `expdate:<formMatch>@<Month>/<YYYY>` — fix a position's start date. Identifies the month/year selects by OPTION SHAPE (13 options non-numeric = month; options matching /^\d{4}$/ = year) — never by position or label, which are language-unstable. Errors are surfaced, not swallowed: a silent `.catch` here once left a new position dated with a stale prefill.
- `skilldelete:<name>` — permanently delete a skill (soft-skill cleanup). Verifies the anchor is actually gone after the confirm click. Endorsements and assessment badges on the skill are lost — never use it on a technical skill worth keeping.
- `certdelete:<name>` — delete a certification, matched by the edit form's FIELD VALUES (the name lives in an input).
- `all` — headline + about + all experiences + all skills from changes.mjs.

Month fields in `certtop:`, `certadd:`, `expadd:`, and `expdate:` accept 1-12 or a month name ("Feb"); invalid months throw BEFORE any delete step (a NaN month once aborted a certtop after its delete, dropping the cert — re-add with `certadd:` if that ever recurs).
...
- `scripts/verify-workplace.mjs` — opens the profile, hunts for the workplace-verification entry, pre-fills the work email and requests the code if reachable, then KEEPS THE BROWSER OPEN for the user to finish (`node verify-workplace.mjs <profileUrl> [email]`; exits when the window closes). The entry point is only reliably present right after adding a position — otherwise it is behind the name-badge shield menu or linkedin.com/verify.
